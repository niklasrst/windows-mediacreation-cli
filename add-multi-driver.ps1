...
# Only allow one vendor for $DriverManufacturer
if ($DriverManufacturer -is [array] -and $DriverManufacturer.Count -gt 1) {
    Write-Error "Only one DriverManufacturer can be specified."
    exit 1
}
...
Write-Verbose "DriverModel(s): $($DriverModel -join ', ')"
    ...
    switch ($DriverManufacturer) {
        "Dell" 
        {
            if ((Test-NetConnection -ComputerName "dell.com" -Port 80).TcpTestSucceeded -ne $true) {
                ...
                Write-Verbose "Searching Dell drivers for $model ..."
                $currentModel = $model
                if ($currentModel -match "\s") {
                    Write-Verbose "Replacing spaces in DriverModel with dashes..."
                    $currentModel = $currentModel -replace '\s', '-'
                }
                ...
                $dellDriverPath = $driversXml.DriverPackManifest.DriverPackage.Path | Where-Object { $_ -match "$currentModel.*$SupportedOsVersion" } | Select-Object -First 1
                $baseDownloadUrl = ""
                $dellDriverUrl = $baseDownloadUrl + $dellDriverPath
                $dellDriverSetup = $dellDriverPath -replace ".*($($currentModel).*)", '$1'

                if ($null -eq $dellDriverUrl) {
                    Write-Error "No Dell driver found for $currentModel. Skipping driver download."
                } else {
                    Write-Verbose "Found Dell driver URL for $currentModel $dellDriverUrl"
                    ...
                    $oemDriverPackDir = (Get-ChildItem -Path $driverpackTempDir -Directory | Where-Object { $_.Name -match "$($currentModel).*" }).FullName
                    
                    switch ($DriverInjectionType) {
                        "AUTOUNATTEND" {
                            Write-Verbose "Copying Dell driver to $UsbDriveLetter\drivers\$($DriverManufacturer)-$($currentModel)..."
                            $oemDriverPackName = (Get-ChildItem -Path $driverpackTempDir -Directory | Where-Object { $_.Name -match "$($currentModel).*" }).Name
                        ...
                        "DISM" {
                            ...
                            "$DriverManufacturer,$currentModel" | Out-File -FilePath $dismDriverDetectionPath -Append -Force
                        }
                        default {
                            $oemDriverPackName = (Get-ChildItem -Path $driverpackTempDir -Directory | Where-Object { $_.Name -match "$($currentModel).*" }).Name
                            $oemDriverMediaDir = $DriverManufacturer + "-" + $oemDriverPackName
        ...
        "Lenovo" 
        {
            if ((Test-NetConnection -ComputerName "lenovo.com" -Port 80).TcpTestSucceeded -ne $true) {
                ...
                Write-Verbose "Searching Lenovo drivers for $model ..."
                $currentModel = $model
                ...
                $lenovoDriverPath = $driversXml.ModelList.Model | Where-Object { $_.name -match $currentModel }
                ...
                if ($null -eq $lenovoDriverUrl) {
                    Write-Error "No Lenovo driver found for $currentModel. Skipping driver download."
                } else {
                    Write-Verbose "Found Lenovo driver URL for $currentModel $lenovoDriverUrl"
                    ...
                    switch ($DriverInjectionType) {
                        "AUTOUNATTEND" {
                            Write-Verbose "Copying Lenovo driver to $UsbDriveLetter\drivers\$($DriverManufacturer)-$($currentModel)..."
                            New-Item -Path "$UsbDriveLetter\drivers\$($DriverManufacturer)-$($currentModel)" -ItemType Directory -Force | Out-Null
                            Copy-Item -Path "$extractDir\*" -Destination "$UsbDriveLetter\drivers\$($DriverManufacturer)-$($currentModel)" -Recurse -Force | Out-Null
                        }
                        "DISM" {
                            Write-Verbose "Injecting drivers to install.wim..."
        ...
        "HP" 
        {
            if ((Test-NetConnection -ComputerName "hp.com" -Port 80).TcpTestSucceeded -ne $true) {
                ...
                Write-Verbose "Searching HP drivers for $model ..."
                $currentModel = $model
                ...
                $hpDriverPath = $driversXml.NewDataSet.HPClientDriverPackCatalog.SoftPaqList.SoftPaq | Where-Object { ($_.name -match $currentModel) -and ($_.name -match "$($SupportedOsVersionFull)") } | Sort-Object id | Select-Object -First 1
                $hpDriverUrl = $hpDriverPath.Url
                $hpDriverSetup = $hpDriverUrl -replace '.*/', ''

                if ($null -eq $hpDriverUrl) {
                    Write-Error "No HP driver found for $currentModel. Skipping driver download."
                } else {
                    Write-Verbose "Found HP driver URL for $currentModel $hpDriverUrl"
                    ...
                    $searchString = $currentModel -replace " ", "*"
                    $extractDir = (Get-ChildItem -Path $driverpackTempDir -Directory | Where-Object { $_.Name -like "*$searchString*" } | Get-ChildItem).FullName

                    switch ($DriverInjectionType) {
                        "AUTOUNATTEND" {
                            Write-Verbose "Copying HP driver to $UsbDriveLetter\drivers\$($DriverManufacturer)-$($currentModel)..."
                            New-Item -Path "$UsbDriveLetter\drivers\$($DriverManufacturer)-$($currentModel)" -ItemType Directory -Force | Out-Null
                            Copy-Item -Path "$($extractDir)\*" -Destination "$UsbDriveLetter\drivers\$($DriverManufacturer)-$($currentModel)" -Recurse -Force | Out-Null
                        }
                        "DISM" {
